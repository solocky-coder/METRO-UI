...
namespace
{
    ...
    static bool writeUsbCgpConfiguration(const std::wstring& parent)
    {
        ...
        wchar_t driver[512] = {};
        bytes = sizeof(driver);
        const auto driverRc = RegQueryValueExW(deviceKey, L"Driver", nullptr, &type, reinterpret_cast<LPBYTE>(driver), &bytes);
        if (driverRc != ERROR_SUCCESS || type != REG_SZ) { RegCloseKey(deviceKey); return false; }
        ...
        if (RegQueryValueExW(deviceKey, L"LowerFilters", nullptr, &lowerType, nullptr, &lowerBytes) == ERROR_SUCCESS && lowerType == REG_MULTI_SZ && lowerBytes > 0)
        {
            ...
            if (RegQueryValueExW(deviceKey, L"LowerFilters", nullptr, &lowerType, reinterpret_cast<LPBYTE>(values.data()), &lowerBytes) == ERROR_SUCCESS)
            {
                ...
                for (const wchar_t* p = values.data(); *p; p += wcslen(p) + 1)
                    if (_wcsicmp(p, L"AppleLowerFilter") != 0) keep.emplace_back(p);
    ...
    static bool selectCompatibleDriver(const std::wstring& instanceId, const std::wstring& token)
    {
        ...
        {
            const std::wstring desc = info.Description;
            const std::wstring provider = info.ProviderName;
            const std::wstring all = desc + L" " + provider;
    ...
    static bool restartDevice(const std::wstring& instanceId)
    {
        ...
        STARTUPINFOW si{}; si.cb = sizeof(si); PROCESS_INFORMATION pi{}; std::vector<wchar_t> line(cmd.begin(), cmd.end()); line.push_back(L'\0');
...
bool AppleUsbNetworkTransport::start(const juce::String& deviceId)
{
    ...
    if (findWinUsbPathForParent(parent).empty()) { currentStatus = "Apple USB control interface is not bound to WinUSB (MI_00); install the DYSEKT WinUSB control driver first"; currentState.store(State::Error, std::memory_order_release); return false; }
    ...
    if (!waitForWinUsb(parent, 8000)) { currentStatus = "Apple USB: WinUSB control interface did not return after re-enumeration"; currentState.store(State::Error, std::memory_order_release); return false; }
    ...
    for (int attempt = 0; attempt < 40 && ncmInterfaces.empty(); ++attempt)
    {
        ...
        if (!parent.empty())
        {
            writeUsbCgpConfiguration(parent);
            setUsbConfigurationHints(parent, 4, 2);
            if (waitForWinUsb(parent, 500)) ncmInterfaces = findInterruptBackedNcmControlInterfaces(parent);
        }
