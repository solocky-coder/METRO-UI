...
namespace
{
    ...
    static std::wstring getDeviceInstanceId(HDEVINFO set, SP_DEVINFO_DATA& data)
    {
        wchar_t buffer[1024] = {};
        DWORD required = 0;
        if (!SetupDiGetDeviceInstanceIdW(set, &data, buffer, ARRAYSIZE(buffer), &required))
            return {};
        return buffer;
    }
    ...
    static std::wstring findWinUsbPath()
    {
        ...
        for (DWORD i = 0; ; ++i)
        {
            ...
            data.cbSize = sizeof(data);
            if (!SetupDiEnumDeviceInterfaces(set, nullptr, &WinUsbInterfaceGuid, i, &data))
            {
                if (GetLastError() == ERROR_NO_MORE_ITEMS) break;
                continue;
            }
    ...
    static bool controlTransfer(WINUSB_INTERFACE_HANDLE usb, UCHAR requestType, UCHAR request,
                                USHORT value, USHORT index, std::vector<UCHAR>& buffer)
    {
    ...
    static std::vector<UCHAR> getDescriptor(WINUSB_INTERFACE_HANDLE usb, UCHAR type, UCHAR index, USHORT length)
    {
        ...
        if (!controlTransfer(usb, 0x80, 0x06, static_cast<USHORT>((type << 8) | index), 0, buffer))
            return {};
        return buffer;
    }
    ...
    static std::vector<int> findInterruptBackedNcmControlInterfaces()
    {
        ...
        auto device = getDescriptor(handle.usb, 0x01, 0, 18);
        ...
        {
            auto head = getDescriptor(handle.usb, 0x02, static_cast<UCHAR>(configuration), 9);
            if (head.size() < 9) continue;
            const int total = head[2] | (head[3] << 8);
            ...
            {
                const auto length = cfg[pos];
                const auto type = cfg[pos + 1];
                if (length < 2 || pos + length > cfg.size()) break;

                if (type == 0x04 && length >= 9 && cfg[pos + 5] == 0x02 && cfg[pos + 6] == 0x0D)
                {
                    const int number = cfg[pos + 2];
                    const int alternate = cfg[pos + 3];
                    const int endpointCount = cfg[pos + 4];
                    bool interruptIn = false;
                    size_t scan = pos + length;
                    int endpoints = 0;
                    while (scan + 2 <= cfg.size() && endpoints < endpointCount)
                    {
                        const auto subLength = cfg[scan];
                        const auto subType = cfg[scan + 1];
                        if (subLength < 2 || scan + subLength > cfg.size() || subType == 0x04) break;
                        if (subType == 0x05 && subLength >= 7)
                        {
                            ++endpoints;
                            const auto address = cfg[scan + 2];
                            const auto attributes = cfg[scan + 3];
    ...
    static bool waitForWinUsb(int timeoutMs)
    {
        ...
        {
            if (!findWinUsbPath().empty()) return true;
            Sleep(100);
        }
        return false;
    }
...
bool AppleUsbNetworkTransport::start(const juce::String& deviceId)
{
    ...
    currentStatus = "Apple USB/NCM transport is Windows-only";
    ...
    if (parent.empty())
    {
        currentStatus = "Apple USB device not present";
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }

    if (findWinUsbPath().empty())
    {
        currentStatus = "Apple WinUSB control interface not present — install WinUsbControl first";
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }
    ...
    if (!restartDevice(parent))
    {
        currentStatus = "Apple USB: device restart after safe configuration failed";
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }

    if (!waitForWinUsb(6000))
    {
        currentStatus = "Apple USB: WinUSB control interface did not return after re-enumeration";
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }
    ...
    {
        currentStatus = "Apple USB: unexpected mode " + juce::String(*mode);
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }
    ...
    if (!setConfiguration(4))
    {
        currentStatus = "Apple USB: SET_CONFIGURATION(4) failed";
        currentState.store(State::Error, std::memory_order_release);
        return false;
    }
    ...
    if (ncmInterfaces.empty())
    {
        ...
        if (!foundMi02)
        {
            currentStatus = "Apple USB: CDC-NCM MI_02 was not published";
            currentState.store(State::Error, std::memory_order_release);
            return false;
        }
    }
