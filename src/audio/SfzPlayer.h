...
private:
...
#if DYSEKT_HAS_FLUIDSYNTH
    // Old synth/settings retired here by the audio thread instead of being
    // deleted inline, so the audio thread never touches
    // FLUIDSYNTH_LIFECYCLE_LOCK(). Drained + actually deleted (under that
    // lock) by the next SynthBuildJob, off the audio thread, before it builds
    // the new instance — see drainRetiredFluidSynths().
    ...
    // Metadata-only lock: its critical section is just a couple of pointer
    // writes — no FluidSynth API calls, no disk I/O, no allocation — so
    // unlike FLUIDSYNTH_LIFECYCLE_LOCK it can never block the audio thread
    // for a meaningful amount of time no matter what a background job is
    // doing. This is what makes it safe for retireFluidSynth() to take from
    // the audio thread.
    std::mutex retireRingMutex;

    void retireFluidSynth (fluid_synth_t* s, fluid_settings_t* st) noexcept;   ///< audio thread
    void drainRetiredFluidSynths();                                           ///< background thread only
#endif

    // Background job that builds a fresh SF2/SFZ engine off the audio
    // thread. Nested so it gets access to SfzPlayer's private members
    // (ReadyEngine, pendingReadyEngine, currentSR/currentBlock, etc.)
