...
void DysektEditor::toggleMultisamplerEditor (bool on)
{
    ...
    if (! on)
        ...
        // onZoneSelectionOrEditChanged just from visibility changing --
        // it's only ever invoked from ZoneMapView's own click handler. On
        // reopen, showMultisamplerEditor is already true above, so the
        // callback's own guard passes; fire it once here so the SCB and
