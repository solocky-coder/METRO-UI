    ...
    : engine (sequencer), linkPtr (link)
{
    ...
    // (already flipped the button on by the time onClick runs) stand until
    // the next timerCallback() tick confirms it. engine.play() only sets a
    // pending-play atomic — isPlaying() doesn't flip true until the next
    // processBlock() — so reading it back here was always false immediately
    // after the click, yanking the toggle back off for one frame before the
    // timer put it right back on.
...
void FloatingTransportBar::timerCallback()
{
    ...
    if (! tempoLabel.isBeingEdited())
    {
        ...
        const float displayBpm = (linkPtr != nullptr && linkPtr->isEnabled())
                                    ...
                                    : engine.getBpm();
        tempoLabel.setText (juce::String (displayBpm, 2), juce::dontSendNotification);
    }
