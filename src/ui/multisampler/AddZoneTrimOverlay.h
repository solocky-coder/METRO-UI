...
public:
    ...
    /** @param sampleFile  file already chosen by the caller's file picker —
     *                     this class never shows its own file chooser.
     *  @param decodePool  background pool the preview decode job runs on.
     *                     Pass DysektProcessor::fileLoadPool; this class
     *                     only ever uses it as a plain juce::ThreadPool and
     *                     never reaches back into the processor otherwise. */
