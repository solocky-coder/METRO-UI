...
private:
    ...
    {
        // Delegates to the shared glyph set in ToolIcons.h — also used by
        // ArrangeView's right-click Tool submenu, so the two views can never
        // visually drift apart. Tool's enum order matches ToolIcons::Kind.
        ToolIcons::draw (g, static_cast<ToolIcons::Kind> (static_cast<int> (tool)), b, colour);
    }
    ...
    {
        return ToolIcons::makeMenuIcon (static_cast<ToolIcons::Kind> (static_cast<int> (tool)), colour);
    }
